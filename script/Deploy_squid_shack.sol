...
import {SquidDinner} from "../src/SquidDinner.sol";
...
contract Deployer is Script {
    ...
    function run() public {
        ...
        bytes24 squidDinner    = registerSquidDinner(ds, extensionID);
        bytes24 squidShack = registerSquidShack(ds, extensionID, squidDinner);

        // dump deployed ids
        console2.log("ItemKind", uint256(bytes32(squidDinner)));
        console2.log("BuildingKind", uint256(bytes32(squidShack)));

        vm.stopBroadcast();
    }

    // register a new item id
    function registerSquidDinner(Game ds, uint64 extensionID) public returns (bytes24 itemKind) {
    ...
    // register a new
    function registerSquidShack(Game ds, uint64 extensionID, bytes24 squidDinner) public returns (bytes24 buildingKind) {
        ...
        return BuildingUtils.register(ds, BuildingConfig({
            ...
            inputs: [
                Input({quantity: 1, item: fleshyBlob}), // these are required inputs to get the output
                Input({quantity: 0, item: none}),
                Input({quantity: 0, item: none}),
                Input({quantity: 0, item: none})
            ],
            outputs: [
                Output({quantity: 1, item: squidDinner}) // this is the output that can be crafted given the inputs
            ],
            implementation: address(new SquidDinner()),
            plugin: vm.readFile("src/SquidDinner.js")
        }));
    }
}
